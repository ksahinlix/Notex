...
- Fonts are ours, not a CDN: `web/public/fonts/` + `web/src/fonts.css`,
  regenerated by `node scripts/fetch-fonts.mjs` (latin + latin-ext only). The
  service worker precaches them, so keep its FONTS list in step.
